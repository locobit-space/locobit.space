  ...
  <div class="shorts-container" ref="containerRef">
      ...
      <TransitionGroup name="slide">
        ...
        >
          ...
          <div class="short-media">
            ...
            <!-- Gradient Overlay -->
            <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-black/20 pointer-events-none" />
          </div>

          <!-- Double Tap Heart Animation -->
          <Transition name="heart">
            <div v-if="showHeartAnimation === short.id" class="heart-animation">
              <Icon name="heroicons:heart-solid" class="w-24 h-24 text-red-500" />
            </div>
          </Transition>
          ...
          <div class="short-content">
            ...
            <div class="creator-section">
              <NuxtLink :to="`/profile/${short.pubkey}`" class="flex items-center gap-3">
                <div class="relative">
                  <UAvatar 
                  ...
                  />
                  <div class="absolute -bottom-1 -right-1 bg-primary-500 rounded-full p-0.5">
                ...
                <div>
                  ...
                  <p class="text-xs text-white/70">@{{ short.userInfo?.name || 'unknown' }}</p>
                </div>
              </NuxtLink>
            ...
            <div class="sound-section" v-if="short.music">
              <div class="flex items-center gap-2 bg-black/30 backdrop-blur-sm rounded-full px-3 py-1.5">
                <Icon name="heroicons:musical-note" class="w-4 h-4 text-white animate-spin-slow" />
          ...
          <div class="action-buttons">
            ...
            <button 
              class="action-btn"
              @click="handleLike(short)"
            >
              <Icon 
                :name="isLiked(short.id) ? 'heroicons:heart-solid' : 'heroicons:heart'" 
                :class="isLiked(short.id) ? 'text-red-500' : 'text-white'"
                class="w-8 h-8"
              />
              <span class="action-count">{{ formatCount(short.likeCount || 0) }}</span>
            </button>
            ...
            >
              <Icon name="heroicons:chat-bubble-oval-left" class="w-8 h-8 text-white" />
              <span class="action-count">{{ formatCount(short.commentCount || 0) }}</span>
            </button>

            <!-- Share -->
            <button 
              class="action-btn"
              @click="openShareSheet(short)"
            >
              <Icon name="heroicons:arrow-path-rounded-square" class="w-8 h-8 text-white" />
              <span class="action-count">{{ formatCount(short.repostCount || 0) }}</span>
            </button>

            <!-- Zap (Bitcoin Lightning) -->
            <button 
              class="action-btn"
              @click="openZapModal(short)"
            >
              <Icon name="lets-icons:lightning-light" class="w-8 h-8 text-yellow-400" />
              <span class="action-count">{{ formatCount(short.zapCount || 0) }}</span>
            </button>

            <!-- Bookmark -->
            <button 
              class="action-btn"
              @click="toggleBookmark(short)"
            >
              <Icon 
                :name="isBookmarked(short.id) ? 'heroicons:bookmark-solid' : 'heroicons:bookmark'" 
                :class="isBookmarked(short.id) ? 'text-yellow-400' : 'text-white'"
                class="w-7 h-7"
              />
            </button>
            ...
            <button 
              class="action-btn"
              @click="openOptions(short)"
            >
              <Icon name="heroicons:ellipsis-horizontal" class="w-7 h-7 text-white" />
            </button>

            <!-- Creator Avatar (Animated Disc) -->
            <NuxtLink :to="`/profile/${short.pubkey}`" class="mt-4">
              <div class="relative">
                <div class="w-12 h-12 rounded-full border-2 border-white overflow-hidden animate-spin-slow">
                  <UAvatar 
                    :src="short.userInfo?.picture" 
                    size="lg"
                    class="w-full h-full"
                ...
                <div class="absolute inset-0 rounded-full border-4 border-gray-900/50"></div>
              </div>
            </NuxtLink>
          </div>
    ...
    <div class="top-nav">
      ...
      <div class="flex gap-4">
        <button 
          :class="{ 'font-bold border-b-2 border-white': feedType === 'following' }"
        ...
        <button 
          :class="{ 'font-bold border-b-2 border-white': feedType === 'for-you' }"
          class="text-white px-2 py-1"
    ...
    <USlideover v-model="showComments" side="bottom" class="h-[70vh]">
      <div class="p-4">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-bold">{{ commentCount }} {{ $t('social.comments') }}</h3>
        ...
        <div class="space-y-4 max-h-[50vh] overflow-y-auto">
          <NoteCommentList v-if="activeShort" :note-id="activeShort.id" :pubkey="activeShort.pubkey" />
        </div>
        
        <div class="mt-4 border-t pt-4">
          <NoteCommentInput v-if="activeShort" :note-id="activeShort.id" :pubkey="activeShort.pubkey" />
        </div>
      </div>
    </USlideover>
      ...
      <div class="p-6">
        ...
        <div class="grid grid-cols-4 gap-4 mb-6">
          ...
          <button class="share-option" @click="shareAction('quote')">
            <div class="share-icon bg-blue-500">
              <Icon name="heroicons:chat-bubble-bottom-center-text" class="w-6 h-6 text-white" />
            </div>
        ...
        <div class="border-t pt-4">
          ...
          <div class="flex gap-3">
            <button class="flex-1 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg text-sm" @click="shareNative">
...
const shareAction = async (type: string) => {
  ...
  switch (type) {
    ...
    case 'quote':
      ...
      break
    case 'copy':
      await navigator.clipboard.writeText(`https://nostr.guru/e/${activeShort.value.id}`)
