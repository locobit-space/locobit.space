  ...
  <div class="stories-container">
    ...
    <div class="stories-scroll" ref="scrollRef">
      ...
      <div class="story-item">
        ...
        >
          ...
          <UAvatar 
            ...
            class="w-full h-full"
          />
          <div v-else class="w-full h-full bg-gray-200 dark:bg-gray-700 rounded-full" />
        </button>
      ...
      >
        ...
        <span class="story-name">{{ story.userInfo?.display_name || 'Anonymous' }}</span>
        <div v-if="story.isLive" class="live-badge">LIVE</div>
      </div>

      <!-- Loading skeleton -->
      <div v-if="isLoading" v-for="i in 5" :key="`skeleton-${i}`" class="story-item">
      ...
      <Transition name="story-modal">
        <div v-if="showViewer" class="story-viewer" @click.self="closeViewer">
          ...
          <div class="story-header">
            <NuxtLink :to="`/profile/${activeStoryGroup?.pubkey}`" class="flex items-center gap-3">
          ...
          <div class="story-actions">
            ...
            <button @click="shareStory" class="reaction-btn">
              <Icon name="heroicons:paper-airplane" class="w-7 h-7 text-white" />
            </button>
          </div>
...
const demoStories = ref([
  {
    pubkey: 'demo1',
    userInfo: { display_name: 'Alice', picture: 'https://i.pravatar.cc/150?u=alice' },
    viewed: false,
    isLive: false,
    items: [
      { id: '1', type: 'image', url: 'https://picsum.photos/400/700?random=1', caption: 'Beautiful sunset 🌅', createdAt: Date.now() - 3600000 },
      { id: '2', type: 'image', url: 'https://picsum.photos/400/700?random=2', caption: '', createdAt: Date.now() - 3000000 },
    ]
  },
  {
    pubkey: 'demo2',
    userInfo: { display_name: 'Bob', picture: 'https://i.pravatar.cc/150?u=bob' },
    viewed: false,
    isLive: true,
    items: [
      { id: '3', type: 'image', url: 'https://picsum.photos/400/700?random=3', caption: 'Check this out!', createdAt: Date.now() - 1800000 },
    ]
  },
  {
    pubkey: 'demo3',
    userInfo: { display_name: 'Carol', picture: 'https://i.pravatar.cc/150?u=carol' },
    viewed: true,
    isLive: false,
    items: [
      { id: '4', type: 'image', url: 'https://picsum.photos/400/700?random=4', caption: '', createdAt: Date.now() - 7200000 },
    ]
  },
])
...
const hasPreviousGroup = computed(() => {
  const currentGroupIndex = groupedStories.value.findIndex(g => g.pubkey === activeStoryGroup.value?.pubkey)
...
const updateProgress = () => {
  if (videoRef.value) {
    storyProgress.value = (videoRef.value.currentTime / videoRef.value.duration) * 100
  }
}

const nextStory = () => {
  if (currentStoryIndex.value < (activeStoryGroup.value?.items?.length || 0) - 1) {
